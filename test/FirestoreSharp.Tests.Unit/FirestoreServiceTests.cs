...
public sealed class FirestoreServiceTests : IClassFixture<WebApplicationFactory<Program>>, IDisposable
{
    ...
    {
        // Use a document path that has no subcollections
        var suffix = Guid.NewGuid().ToString("N")[..8];
    ...
    public async Task ListCollectionIds_WithPageSize_PaginatesResults()
    {
        ...
        // There are other collections in the store so page 1 should have exactly 2 and a token
        Assert.Equal(2, page1.CollectionIds.Count);
        Assert.Collection(page1.CollectionIds, id => Assert.StartsWith($"lcids-01", id), id => Assert.StartsWith($"lcids-02", id));
        Assert.NotEmpty(page1.NextPageToken);
        ...
        // No collection ID should appear on both pages
